...
document.addEventListener('DOMContentLoaded', () => {
        ...
        const handleFirebaseError = (err) => { 
            let title = 'একটি ত্রুটি ঘটেছে';
            let message = err.message || 'একটি অজানা সমস্যা হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।';
            ...
            switch (err.code) {
                ...
                case 'auth/user-not-found':
                    message = 'এই ইমেইল দিয়ে কোনো একাউন্ট পাওয়া যায়নি।';
                    break;
                case 'auth/email-already-in-use':
                    message = 'এই ইমেইলটি ஏற்கனவே ব্যবহার করা হয়েছে।';
                    break;
                case 'auth/weak-password':
                    message = 'পাসওয়ার্ডটি খুবই দুর্বল। অনুগ্রহ করে কমপক্ষে ৬ অক্ষরের পাসওয়ার্ড দিন।';
                    break;
                case 'auth/invalid-email':
                    message = 'আপনার ইমেইল ঠিকানাটি সঠিক নয়।';
                    break;
            }
            showModal('error', title, message, 'ঠিক আছে');
        };
        ...
        function renderPremiumProfile(user, userData) {
            ...
            if (userData.equippedProfileBadge && allDbBadges[userData.equippedProfileBadge]) {
                const badgeData = allDbBadges[userData.equippedProfileBadge];
                if (badgeData.type === 'profile') {
                    badgeHTML = `<img src="${badgeData.url}" class="verification-badge" data-badge-index="${userData.equippedProfileBadge}">`;
                }
            }
            usernameEl.innerHTML = `<span>${name}</span>${badgeHTML}`;
        ...
        async function cleanupExpiredCoupons() {
            if (!currentUserData?.coupons || !auth.currentUser) return;
            ...
            Object.entries(currentUserData.coupons).forEach(([couponId, coupon]) => {
                const expiryTime = new Date(coupon.expiryDate).getTime();
                if (expiryTime + sevenDays < now) {
                    updates[`/users/${auth.currentUser.uid}/coupons/${couponId}`] = null;
                    needsUpdate = true;
                }
            });
            ...
            if (needsUpdate) {
                try {
                    ...
                    console.log("Cleaned up old expired coupons.");
                } catch (error) {
        ...
        function setupRealtimeListener(user) {
            database.ref().on('value', async snapshot => {
                ...
                const renderFunctionMap = {
                    ...
                    'history-content': renderHistoryPage,
                    'redeem-content': renderRedeemPage,
                    'profile-content': () => { renderPremiumProfile(user, currentUserDataFromDb); },
        ...
        function setupAuthPageListeners() {
            ...
            switchAuthView = function(showSignup) {
                const loginView = document.getElementById('login-view');
                const signupView = document.getElementById('signup-view');
        ...
        document.getElementById('signup-form-element').addEventListener('submit', (e) => {
            ...
            handleFormSubmit(form, 'Signing Up...', async () => {
                ...
                if (!email.toLowerCase().endsWith('@gmail.com')) { throw { message: 'শুধুমাত্র @gmail.com ঠিকানা দিয়ে রেজিস্ট্রেশন করা যাবে।' }; }
                const password = form.querySelector('#signup-password').value;
                if (password !== form.querySelector('#signup-confirm-password').value) { throw { message: 'পাসওয়ার্ড দুটি মিলছে না।' }; }

                const countryCode = form.querySelector('#signup-country-code').value;
                const whatsappNumber = form.querySelector('#signup-whatsapp').value.trim();
                if (whatsappNumber.length < 5 || whatsappNumber.length > 15) {
                    throw { message: 'অনুগ্রহ করে একটি সঠিক ওয়াটসঅ্যাপ নম্বর দিন (৫-১৫ ডিজিটের মধ্যে)।' };
                }
                ...
                if (emailExistsSnapshot.exists()) {
                    ...
                    if (existingUser.role === 'sponsor') {
                        throw { message: 'এই ইমেইলটি ইতিমধ্যেই একজন স্পনসর হিসেবে রেজিস্টার করা আছে। অনুগ্রহ করে স্পনসর প্যানেল ব্যবহার করুন।' };
                    } else {
                ...
                showCustomModal({
                    ...
                    message: 'LEVEL UP-এ আপনাকে স্বাগতম! চালিয়ে যেতে লগইন করুন।',
                    primaryButton: {
                        text: 'লগইন করুন',
                        closeOnClick: false, // Prevent default close behavior
                        onClick: () => {
                            const modalScreen = document.getElementById('modal-screen');
                            if (!modalScreen) return;
                            gsap.to(modalScreen.querySelector('.modal-box'), {
        ...
        function updateAllCountdowns() {
            document.querySelectorAll('.countdown-container, .claim-countdown, #season-countdown-timer, #season-details-countdown-timer, .ad-cooldown-timer, .coupon-expiry-timer, .mail-delete-timer, .team-ban-timer, .item-timer, [data-countdown-to]').forEach(c => {
                ...
                if (diff <= 0) {
                    if (c.closest('.my-matches-tab-content')) {
                        const postId = c.closest('[data-post-id]').dataset.postId;
                        if(postId) renderMyMatchesPage(); 
                    }
        ...
        function setupSlider(sliderData) {
            ...
            sC.addEventListener('touchend', touchEnd);
            sC.addEventListener('mousemove', touchMove); sC.addEventListener('touchmove', touchMove, { passive: true });
            ...
            showCustomModal({
                ...
                message: `
                    <div class="space-y-4 w-full">
                        <button id="fab-need-guild" class="w-full text-left p-4 bg-gray-700/50 hover:bg-gray-600/50 rounded-xl transition-all flex items-center gap-4 border border-gray-600 hover:border-green-500">
                            <div class="w-12 h-12 bg-green-500/20 text-green-300 rounded-lg flex items-center justify-center text-2xl flex-shrink-0"><i class="fa-solid fa-shield-halved"></i></div>
                            <div>
                                <p class="font-bold text-lg text-white">Need Guild Members</p>
                                <p class="text-xs text-gray-400">Recruit players for your guild.</p>
                            </div>
                        </button>
                        <button id="fab-need-player-squad" class="w-full text-left p-4 bg-gray-700/50 hover:bg-gray-600/50 rounded-xl transition-all flex items-center gap-4 border border-gray-600 hover:border-blue-500">
                            <div class="w-12 h-12 bg-blue-500/20 text-blue-300 rounded-lg flex items-center justify-center text-2xl flex-shrink-0"><i class="fa-solid fa-user-plus"></i></div>
                            <div>
                                <p class="font-bold text-lg text-white">Need Player / Squad</p>
                                <p class="text-xs text-gray-400">Find a team or recruit individuals.</p>
                            </div>
                        </button>
                    </div>
                `,
    ...
    // [UPDATED] Separated prize claims from history
    function renderMyMatchesPage() {
        ...
        Object.values(bookings).forEach(booking => {
            ...
            const hasWinnings = post.winnings && post.winnings[auth.currentUser.uid];
            const isClaimed = hasWinnings && post.winnings[auth.currentUser.uid].claimed;

            if (post.status === 'completed') {
                if (hasWinnings && !isClaimed) {
                    claimableMatches.push({ post, isBanned, booking });
                } else {
                    const eventTime = new Date(post.completedAt || post.timestamp).getTime();
                    if (eventTime > sevenDaysAgo) {
                         historyMatches.push({ post, isBanned, booking });
                    }
                }
            } else if (post.status.startsWith('live')) {
                liveMatches.push({ post, isBanned, booking });
            } else if (post.status === 'upcoming') {
                upcomingMatches.push({ post, isBanned, booking });
            } else if (post.status === 'cancelled') {
                 const eventTime = new Date(post.cancelledAt || post.timestamp).getTime();
                 if (eventTime > sevenDaysAgo) {
                    historyMatches.push({ post, isBanned, booking });
                 }
            }
        ...
        upcomingContainer.innerHTML = upcomingMatches.length > 0 ? upcomingMatches.map(item => createMyMatchPostCardHTML(item.post, item.isBanned, item.booking)).join('') : '<p class="text-gray-500 p-4 text-center">You have no upcoming matches.</p>';
        liveContainer.innerHTML = liveMatches.length > 0 ? liveMatches.map(item => createMyMatchPostCardHTML(item.post, item.isBanned, item.booking)).join('') : '<p class="text-gray-500 p-4 text-center">No matches are live right now.</p>';
        claimableContainer.innerHTML = claimableMatches.length > 0 ? claimableMatches.map(item => createMyMatchPostCardHTML(item.post, item.isBanned, item.booking)).join('') : '<p class="text-gray-500 p-4 text-center">No prizes to claim.</p>';
        historyContainer.innerHTML = historyMatches.length > 0 ? historyMatches.map(item => createMyMatchPostCardHTML(item.post, item.isBanned, item.booking)).join('') + '<p class="text-center text-gray-600 mt-4 text-sm">ম্যাচের হিস্টোরি ৭ দিন পর স্বয়ংক্রিয়ভাবে মুছে যাবে।</p>' : '<p class="text-center text-gray-500 p-4 text-center">No recent match history found.</p>';
        
        initializeCountdowns();
    }
    
    // [UPDATED] Renders "Claim Reward" button instead of countdown/timer
    function createMyMatchPostCardHTML(post, isBanned = false, booking) {
        ...
        if (isBanned) {
            ...
            if (hasWinnings && !isClaimed) {
                const prizeData = post.winnings[auth.currentUser.uid];
                bottomSectionHTML = `<div class="p-3 bg-green-900/50 text-center">
                                        <p class="text-sm">You won: <strong class="text-xl font-bold text-green-400">${prizeData.amount.toFixed(2)} TK</strong></p>
                                        <button class="w-full mt-2 py-2 action-button rounded-lg claim-reward-btn" data-post-id="${post.id}">Claim Reward</button>
                                    </div>`;
            } else {
                const entryFee = booking.finalFee !== undefined ? booking.finalFee : 0;
                const winningsAmount = hasWinnings && isClaimed ? post.winnings[auth.currentUser.uid].amount : 0;
                 bottomSectionHTML = `<div class="p-3 bg-gray-800/80">
                                        <p class="text-center font-bold text-gray-300 mb-2">MATCH COMPLETED</p>
                                        <div class="flex justify-around items-center text-center">
                                            <div>
                                                <p class="text-xs text-gray-400">Join Fee</p>
                                                <p class="text-lg font-bold text-red-400">-${entryFee.toFixed(2)}</p>
                                            </div>
                                            <i class="fa-solid fa-arrow-right-long text-gray-500"></i>
                                            <div>
                                                <p class="text-xs text-gray-400">Winning</p>
                                                <p class="text-lg font-bold text-green-400">+${winningsAmount.toFixed(2)}</p>
                                            </div>
                                        </div>
                                     </div>`;
            }
    ...
    document.getElementById('my-matches-content').addEventListener('click', e => { 
        ...
        if (claimBtn) {
            e.stopPropagation();
            showPrizeClaimFeedbackModal(claimBtn.dataset.postId); // [UPDATED] Trigger feedback modal instead of direct claim
            return;
        }
    ...
    function renderCSHistoryPage(postId) {
        ...
        const leaderboardImage = post.leaderboardImage; // Check if sponsor uploaded a custom LB image
        ...
        const imageToDisplay = leaderboardImage || resultImage;
        let imageSection = '';
        
        if (imageToDisplay) {
            imageSection = `
                <div class="leaderboard-download-section mt-6">
                    <h4 class="font-bold text-gray-300 mb-2 text-sm uppercase tracking-wider text-center">Match Result / Leaderboard</h4>
                    <img src="${imageToDisplay}" class="w-full h-auto object-contain rounded-lg cursor-pointer" onclick="event.stopPropagation(); showLightbox('${imageToDisplay}')" loading="lazy">
                    <button class="download-lb-btn" data-img-url="${imageToDisplay}">
        ...
        // Return string, event listener needs to be attached after insertion
        return content;
    }

    // [UPDATED] Separated prize claim section from main history view
    function renderHistoryMatchDetailsPage(postId) {
        ...
        if (post.details.matchType === 'Clash Squad') {
            ...
            // Attach listener for download button in CS page
            const downloadBtn = content.querySelector('.download-lb-btn');
            if(downloadBtn) {
        ...
        let prizeHTML = '';
        if (hasWinnings && !isClaimed) {
             prizeHTML = `<div id="prize-claim-section" class="mt-8 p-4 bg-green-900/50 border border-green-700 rounded-xl"></div>`;
        } else if (isClaimed) {
            const prizeData = post.winnings[auth.currentUser.uid];
            prizeHTML = `<div class="mt-8 p-4 bg-gray-800 rounded-xl text-center">
                            <p class="text-lg">You claimed: <strong class="text-2xl font-bold text-green-400">${prizeData.amount.toFixed(2)} TK</strong></p>
                        </div>`;
        } else {
             prizeHTML = `<p class="text-center text-gray-500 mt-8">Sorry, you did not win a prize in this match.</p>`;
        }
        ...
        // NEW: Leaderboard Image Section
        let leaderboardImageHTML = '';
        if (post.leaderboardImage) {
            leaderboardImageHTML = `
                <div class="leaderboard-download-section">
                    <h4 class="font-bold text-gray-300 mb-2 text-sm uppercase tracking-wider text-center">Official Result Table</h4>
                    <img src="${post.leaderboardImage}" class="w-full h-auto rounded-lg mb-3 border border-gray-600 cursor-pointer" onclick="showLightbox('${post.leaderboardImage}')" loading="lazy">
                    <button class="download-lb-btn" data-img-url="${post.leaderboardImage}">
                        <i class="fa-solid fa-download"></i> Download Leaderboard
                    </button>
                </div>
            `;
        }
        ...
        if (hasWinnings && !isClaimed) {
            renderPrizeClaimSection(postId);
        }
        if (!hasRated) {
    ...
    async function handleImageDownload(e) {
        ...
        try {
            ...
            a.download = `Leaderboard-${Date.now()}.jpg`;
    ...
    async function handleFollowToggle(targetUserId, buttonElement = null) {
        const followBtn = buttonElement || document.getElementById('user-profile-follow-toggle-btn');
        ...
        if (isCurrentlyFollowing) {
            ...
            // Notification logic
            const newMailKey = database.ref(`/users/${targetUserId}/mailbox`).push().key;
            const isFollowingBack = currentUserData.followers && currentUserData.followers[targetUserId];
            const subject = isFollowingBack ? "Followed you back!" : "New Follower!";
            const message = isFollowingBack 
                ? `${currentUserData.name} followed you back.`
                : `${currentUserData.name} started following you.`;
    
            updates[`/users/${targetUserId}/mailbox/${newMailKey}`] = {
                id: newMailKey,
    ...
    async function renderMailbox() {
        ...
        content.querySelectorAll('.mail-item-premium').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                const followerId = item.dataset.followerId;
                if (followerId) {
                    handleNavigation('user-profile-content');
                    renderUserProfile(followerId);
                    return; 
                }
    ...
    function createMailItemHTML(mail) {
        ...
        const followerIdAttr = mail.type === 'follow_notification' ? `data-follower-id="${mail.body.followerId}"` : '';

        if(mail.type === 'reward' && Array.isArray(mail.body.rewards)) {
            ...
            const rewardsHTML = mail.body.rewards.map(reward => {
                ...
                if (reward.rewardType === 'coin' || reward.rewardType === 'diamond') {
                    ...
                    name = `${reward.rewardValue} ${reward.rewardType.charAt(0).toUpperCase() + reward.rewardType.slice(1)}s`;
                } else {
                    ...
                    name = itemData?.title || 'Special Item';
                ...
                return `<div class="reward-grid-item">
                            ${durationHTML}
                            <div class="reward-grid-item-media-wrapper">
                                <${mediaTag} src="${rewardIcon}" class="reward-grid-item-media" ${mediaTag === 'video' ? 'autoplay loop muted playsinline' : 'loading="lazy"'}></${mediaTag}>
                            </div>
                            <p class="reward-grid-item-name">${name}</p>
                        </div>`;
            }).join('');

            bodyContent = `
            <p class="text-sm text-gray-300 mb-4">${mail.body.message || 'You have received rewards!'}</p>
            <div class="reward-grid-container">${rewardsHTML}</div>`;
            ...
            switch(mail.type) {
                ...
                case 'follow_notification':
                    iconClass = 'fa-user-plus text-blue-400';
                    bodyContent = `<p class="text-sm text-gray-300">${mail.body.message}</p>`;
                    break;
                default:
        ...
        return `
        <div class="mail-item-premium ${mail.read ? '' : 'unread'}" data-mail-id="${mail.id}" ${followerIdAttr}>
            <div class="p-4">
    ...
    async function handleClaimReward(mailId, button) {
        ...
        try {
            if(mail.body.rewards) {
                ...
                successMessage = `You have received a ${mail.body.rewardName || 'reward'}!`;
            }
...
function setupProfilePageListeners() {
        document.body.addEventListener('click', e => {
            ...
            if (e.target.closest('#create-team-button') || e.target.closest('#my-team-card-clickable-wrapper')) {
                myTeamClickHandler();
            }
        });
    ...
    function renderHomeGuildPosts() {
        ...
        const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
        const activePosts = allGuildPosts.filter(post => new Date(post.timestamp).getTime() > thirtyDaysAgo).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
        
        if (activePosts.length === 0) { 
            container.innerHTML = `<div class="text-center text-gray-500 p-10 bg-gray-800/50 rounded-lg"><i class="fa-solid fa-users-slash text-4xl mb-4"></i><p>কোন গিল্ড পোস্ট এখন উপলব্ধ নেই।</p></div>`; 
            return; 
        }
    ...
    function createGuildPostCardHTML(post) {
        const alreadyApplied = post.applications && post.applications[auth.currentUser.uid]; 
        const isMyPost = post.creatorId === auth.currentUser.uid; 
        let buttonHTML;
        
        if(isMyPost) { 
            buttonHTML = `<button class="manage-my-guild-post-btn w-full py-3 rounded-lg bg-blue-600 hover:bg-blue-700 font-bold text-sm">Manage Post</button>`;
        } else if(alreadyApplied) { 
            buttonHTML = `<button class="w-full py-3 rounded-lg bg-yellow-600 font-bold text-sm" disabled>আবেদন করা হয়েছে</button>`;
        } else { 
            buttonHTML = `<button class="guild-apply-btn w-full py-3 rounded-lg action-button font-bold text-sm" data-guild-post-id="${post.id}">আমি জয়েন হতে চাই</button>`; 
        }
    ...
    function renderUserProfile(userId) {
        ...
        if (userId === auth.currentUser.uid) {
            ...
            const isFollowedBy = currentUserData.followers && currentUserData.followers[userId];
            if (isFollowing) {
    ...
    function renderLupInventory() {
        ...
        const allItems = {
            ...
            titles: { ...window.dbData.admin?.config?.titles },
        ...
        const profileTitles = Object.keys(currentUserData.earnedTitles || {}).map(id => ({ id, ...allItems.titles[id], itemType: 'title', expiry: currentUserData.earnedTitles[id] })).filter(t => t && t.url);
        const profileFrames = Object.keys(currentUserData.inventory?.frames || {}).map(id => ({ id, ...allItems.frames[id], itemType: 'frame', expiry: currentUserData.inventory.frames[id] })).filter(f => f && f.url);
        const profileEffects = Object.keys(currentUserData.inventory?.effects || {}).map(id => ({ id, ...allItems.effects[id], itemType: 'profile_effect', expiry: currentUserData.inventory.effects[id] })).filter(e => e && e.url);
        ...
        const createSection = (title, items, equippedId, gridClass) => {
            ...
            const itemsHTML = items.map(item => {
                ...
                const media = (item.type === 'video' || (item.url && item.url.endsWith('.mp4')))
...
function renderRoomDetailsTab(postId) {
    ...
    // [UPDATED] Separated prize claim from the main history page logic
    function renderPrizeClaimSection(postId) {
        const post = window.dbData.posts[postId];
        const container = document.getElementById('prize-claim-section');
        ...
        if (hasWinnings && !isClaimed) {
            ...
            container.innerHTML = `
                <div class="text-center">
                    <p class="text-sm text-gray-400">Your Rank: <strong class="text-white">#${prizeData.rank}</strong> | Your Points: <strong class="text-white">${prizeData.points || 'N/A'}</strong></p>
                    <p class="text-lg mt-2">Prize Won: <strong class="text-2xl font-bold text-green-400">${prizeData.amount.toFixed(2)} TK</strong></p>
                    <div class="mt-4">
                        <button class="w-full py-3 action-button rounded-lg claim-reward-btn" data-post-id="${postId}">Claim Reward</button>
                    </div>
                </div>`;
            container.querySelector('.claim-reward-btn')?.addEventListener('click', (e) => showPrizeClaimFeedbackModal(postId));
        }
    }
    
// [NEW] Function to show a modal requiring feedback before claiming a prize
function showPrizeClaimFeedbackModal(postId) {
    const post = window.dbData.posts[postId];
    const prizeData = post.winnings[auth.currentUser.uid];
    ...
    showCustomModal({
        ...
        message: `
            <div id="claim-feedback-container" class="w-full text-left">
                <p class="text-center text-gray-300 mb-4">You won <strong class="text-green-400">${prizeData.amount.toFixed(2)} TK</strong>! Please provide your feedback for the event to claim your prize.</p>
                <h3 class="font-bold text-lg mb-2 text-center">Rate This Event</h3>
                <div class="flex justify-center text-3xl gap-2 rating-stars mb-3" data-rating="0">${Array(5).fill(0).map((_, i) => `<i class="fa-regular fa-star" data-value="${i+1}"></i>`).join('')}</div>
                <textarea id="feedback-comment" class="form-textarea" rows="3" placeholder="Write your feedback (optional)..."></textarea>
            </div>`,
        primaryButton: { 
            text: 'Submit & Claim Prize', 
            closeOnClick: false,
            onClick: () => {
                const modal = document.getElementById('modal-screen');
                const rating = parseInt(modal.querySelector('.rating-stars').dataset.rating);
                const comment = modal.querySelector('#feedback-comment').value.trim();
                
                if (rating === 0) {
                    showModal('error', 'Rating Required', 'Please select at least one star to claim your prize.');
                    return; // Keep modal open
                }
                
                // Now call handleClaimPrize with the collected data
                handleClaimPrize(postId, rating, comment);
            }
        },
        onRender: (modal) => {
            ...
            stars.forEach(star => {
                ...
                star.addEventListener('mouseout', () => highlightStars(stars, modal.querySelector('.rating-stars').dataset.rating));
                star.addEventListener('click', () => {
                    modal.querySelector('.rating-stars').dataset.rating = star.dataset.value;
                    highlightStars(stars, star.dataset.value); // Ensure selection persists visually
                });
            });
        }
    });
...
// [MODIFIED] handleClaimPrize now accepts feedback data as arguments
async function handleClaimPrize(postId, rating, comment) {
    const post = window.dbData.posts[postId];
    const prizeData = post.winnings[auth.currentUser.uid];
    const prizeAmount = prizeData.amount;
    
    if (!post || prizeData.claimed) return;

    showLoader('Submitting Feedback & Claiming...');

    try {
        // Submit feedback first
        const newFeedbackKey = database.ref(`posts/${postId}/feedback`).push().key;
        const newFeedback = { userId: auth.currentUser.uid, rating, comment, timestamp: new Date().toISOString() };
        
        // Then process prize claim
        const updates = {};
        updates[`/posts/${postId}/feedback/${newFeedbackKey}`] = newFeedback;
        updates[`/users/${auth.currentUser.uid}/wallet/live`] = firebase.database.ServerValue.increment(prizeAmount);
        updates[`/posts/${postId}/winnings/${auth.currentUser.uid}/claimed`] = true;
        ...
        // Manually close the feedback modal before showing success
        const feedbackModal = document.getElementById('modal-screen');
        if (feedbackModal) {
            gsap.to(feedbackModal.querySelector('.modal-box'), {
                scale: 0.8, opacity: 0, y: 50, duration: 0.3, ease: "power2.in",
        ...
        showCustomModal({
            type: 'success',
            title: 'Congratulations!',
            message: `${prizeAmount.toFixed(2)} TK has been added to your Live Wallet. Thank you for your feedback!`,
            primaryButton: { text: 'Awesome!' }
        });
...
// [MODIFIED] Re-purposed for general feedback, now also called by the prize claim modal
function renderFeedbackForm(postId) {
    const container = document.getElementById('feedback-form-container');
    if (!container) return;
    container.innerHTML = `
        <h3 class="font-bold text-lg mb-2">Rate This Event</h3>
        <div class="flex justify-center text-2xl gap-2 rating-stars mb-3" data-rating="0">${Array(5).fill(0).map((_, i) => `<i class="fa-regular fa-star" data-value="${i+1}"></i>`).join('')}</div>
        <textarea id="feedback-comment" class="form-textarea" rows="3" placeholder="Write your feedback (optional)..."></textarea>
        <button id="submit-feedback-btn" class="w-full py-2 mt-2 action-button rounded-lg">Submit Feedback</button>`;

    const stars = container.querySelectorAll('.rating-stars .fa-star');
    stars.forEach(star => {
        star.addEventListener('mouseover', () => highlightStars(stars, star.dataset.value));
        star.addEventListener('mouseout', () => highlightStars(stars, container.querySelector('.rating-stars').dataset.rating));
        star.addEventListener('click', () => {
            const ratingValue = star.dataset.value;
            container.querySelector('.rating-stars').dataset.rating = ratingValue;
            highlightStars(stars, ratingValue); // Keep stars highlighted on click
        });
    });

    document.getElementById('submit-feedback-btn').addEventListener('click', async (e) => {
        const btn = e.currentTarget;
        const rating = parseInt(document.querySelector('#feedback-form-container .rating-stars').dataset.rating);
        const comment = document.getElementById('feedback-comment').value.trim();
        
        if (rating === 0) {
            showModal('error', 'Rating required', 'Please select at least one star to submit feedback.');
            return;
        }
        ...
        try {
            await submitFeedback(postId, rating, comment);
            // The submitFeedback function will now handle the UI update.
        } catch (error) {
            handleFirebaseError(error);
        } finally {
            hideLoader();
...
// [MODIFIED] Now accepts data, handles UI update, and loading state
async function submitFeedback(postId, rating, comment) {
    const newFeedbackKey = database.ref(`posts/${postId}/feedback`).push().key;
    const newFeedback = { userId: auth.currentUser.uid, rating, comment, timestamp: new Date().toISOString() };
    
    await database.ref(`posts/${postId}/feedback/${newFeedbackKey}`).set(newFeedback);

    // Update the UI immediately after successful submission
    const container = document.getElementById('feedback-form-container');
    if (container) {
        container.innerHTML = `<div class="p-4 bg-[#1e1e1e] rounded-xl text-center text-green-400 mt-6"><i class="fa-solid fa-check-circle mr-2"></i>Thank you for your feedback!</div>`;
    }
